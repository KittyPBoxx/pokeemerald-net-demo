...
static const u8 sNetGameName[] = _("FR/LG Net 0.1.3      ");
...
static void Task_LinkupProcess(u8 taskId)
{
    switch (sSendRecvMgr.nextProcessStep)
    {
        ...
        case LINKUP_REQUEST_NETWORK_STATUS:
            ...
            configureSendRecvMgr(NET_CONN_LIFN_REQ, (vu32 *) &gStringVar4[0], 4, NET_CONN_STATE_RECEIVE, LINKUP_HANDLE_NETWORK_STATUS_OUTCOME);
            break;

        case LINKUP_HANDLE_NETWORK_STATUS_OUTCOME:
            if (!(NET_CONN_LIFN_REQ >> 8 == gStringVar4[0] && (NET_CONN_LIFN_REQ & 0x00FF) == gStringVar4[1]))
            {
                sSendRecvMgr.nextProcessStep = LINKUP_WAIT_FOR_SERVER_TO_CONNECT;
            }
            else if (gStringVar4[3] >= NETWORK_MIN_ERROR)
            {
                gSpecialVar_0x8003 = NET_STAT_ATTACHED_NO_INTERNET;
                sSendRecvMgr.nextProcessStep = LINKUP_FINISH;
            }
            else if (gStringVar4[3] == NETWORK_CONNECTION_SUCCESS && gStringVar4[2] == NETWORK_STATE_WAITING)
            {
        ...
        case LINKUP_RECEIVE_WELCOME_MESSAGE:
            ...
            if (sSendRecvMgr.repeatedStepCount == 0)
            {
                ...
                StringCopy(gStringVar4, sServerName);
            }
            configureSendRecvMgrChunked(NET_CONN_RCHF0_REQ, (vu32 *) &gStringVar4[SERVER_NAME_LENGTH], WELCOME_MSG_LENGTH, NET_CONN_STATE_RECEIVE, LINKUP_FINISH, MINIMUM_CHUNK_SIZE);
            break;

        case LINKUP_FINISH:
        default:
            gStringVar4[SERVER_NAME_LENGTH + WELCOME_MSG_LENGTH] = 0xFF; // Force the message to end if they never never terminated it 
...
static void Task_DownloadBattleProcess(u8 taskId)
{
    switch (sSendRecvMgr.nextProcessStep)
    {
        case DOWNLOAD_BATTLE_SEND_REQUEST: // Puts request data on the wii  (at address channel 2)
            gStringVar4[0] = 'B'; gStringVar4[1] = 'A'; gStringVar4[2] = '_'; gStringVar4[3] = '1'; // The '1' at the end is for if we want multiple downloadable trainers
            configureSendRecvMgr(NET_CONN_SCH2_REQ, (vu32 *) &gStringVar4[0], 4, NET_CONN_STATE_SEND, DOWNLOAD_BATTLE_TRANSMIT_REQUEST);
            break;
        ...
        case DOWNLOAD_BATTLE_RECIEVE_DATA: // Pull back the data from the wii (reading at address chanel 0x0F and writing to gStringVar4)
            sSendRecvMgr.disableChecks = FALSE;
            if (sSendRecvMgr.repeatedStepCount == 0)
            {
                sSendRecvMgr.retryPoint = DOWNLOAD_BATTLE_RECIEVE_DATA;
            }
            configureSendRecvMgrChunked(NET_CONN_RCHF0_REQ, (vu32 *) &gStringVar4[0], DOWNLOAD_TRAINER_POKEMON_SIZE * DOWNLOAD_TRAINER_PARTY_SIZE, NET_CONN_STATE_RECEIVE, DOWNLOAD_BATTLE_FINISH, MINIMUM_CHUNK_SIZE);
            break;


        case DOWNLOAD_BATTLE_FINISH: // Process the data (create the ereader data from what is now stored in gStringVar4)
        default:
        {
            u32 i;
            u32 offset = 0;
            ...
            StringFill(gSaveBlock2Ptr->battleTower.ereaderTrainer.name, CHAR_SPACER, PLAYER_NAME_LENGTH);
            StringCopy_PlayerName(gSaveBlock2Ptr->battleTower.ereaderTrainer.name, trainerName);
            gSaveBlock2Ptr->battleTower.ereaderTrainer.trainerClass = FACILITY_CLASS_PKMN_TRAINER_BRENDAN;           
            ...
            for (i = 0; i < PARTY_SIZE; i++)
            {
                if (i < DOWNLOAD_TRAINER_PARTY_SIZE) 
                {
                    ...
                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].species  = (u16) (gStringVar4[ 0 + offset] | gStringVar4[ 1 + offset] << 8);
                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].level    =        gStringVar4[ 2 + offset]                                 ;
                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].heldItem = (u16) (gStringVar4[ 3 + offset] | gStringVar4[ 4 + offset] << 8);
                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].moves[0] = (u16) (gStringVar4[ 5 + offset] | gStringVar4[ 6 + offset] << 8);
                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].moves[1] = (u16) (gStringVar4[ 7 + offset] | gStringVar4[ 8 + offset] << 8);
                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].moves[2] = (u16) (gStringVar4[ 9 + offset] | gStringVar4[10 + offset] << 8);
                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].moves[3] = (u16) (gStringVar4[11 + offset] | gStringVar4[12 + offset] << 8);

                    StringFill(gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].nickname, CHAR_SPACER, POKEMON_NAME_LENGTH);

                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].nickname[0] = gStringVar4[13 + offset];
                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].nickname[1] = gStringVar4[14 + offset];
                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].nickname[2] = gStringVar4[15 + offset];
                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].nickname[3] = 0xFF;
                    ...
                    if (gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].species > NUM_SPECIES || 
                        gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].level > MAX_LEVEL || 
                        gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].heldItem > ITEMS_COUNT || 
                        gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].moves[0] > MOVES_COUNT || 
                        gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].moves[1] > MOVES_COUNT || 
                        gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].moves[2] > MOVES_COUNT || 
                        gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].moves[3] > MOVES_COUNT )
                    {
                        gSpecialVar_0x8003 = 0;
                    }
                }
                ...
                {
                    gSaveBlock2Ptr->battleTower.ereaderTrainer.party[i].species = 0;
                }

                // Basic validation to make sure we got something sensible back from the server/wii
                if (gSaveBlock2Ptr->battleTower.ereaderTrainer.party[0].species == 0)
                    gSpecialVar_0x8003 = 0;
...
static void Task_OnlineMartProcess(u8 taskId)
{
    switch (sSendRecvMgr.nextProcessStep)
    {
        case DOWNLOAD_MART_SEND_REQUEST:
            gStringVar4[0] = 'M'; gStringVar4[1] = 'A'; gStringVar4[2] = '_'; gStringVar4[3] = '1';
            configureSendRecvMgr(NET_CONN_SCH2_REQ, (vu32 *) &gStringVar4[0], 4, NET_CONN_STATE_SEND, DOWNLOAD_MART_TRANSMIT_REQUEST);
            break;
        ...
        case DOWNLOAD_MART_RECEIVE_DATA:
            ...
            configureSendRecvMgrChunked(NET_CONN_RCHF0_REQ, (vu32 *) &gStringVar4[0], 16, NET_CONN_STATE_RECEIVE, DOWNLOAD_MART_FINISH, MINIMUM_CHUNK_SIZE);
            break;
        ...
        default:
        {
            ...
            {
                if ((u16) (gStringVar4[i*2] | gStringVar4[(i*2) + 1] << 8) > (ITEMS_COUNT + 2)) // +2 is because of emerald items, server needs changing so it wont send them to FRLG
                    gSpecialVar_0x8003 = 0;
            }
            sSendRecvMgr.state = NET_CONN_STATE_DONE;
            break;
...
static void Task_EndOnlineMartConnection(u8 taskId) 
{
    ...
    if (gSpecialVar_0x8003 == 1)
    {
        gStringVar4[12] = 0;
        gStringVar4[13] = 0; 
        CreatePokemartMenu((u16 *) &gStringVar4[0]);
        ScriptContext_Stop();
    }
...
static void Task_GiftEggProcess(u8 taskId)
{
    switch (sSendRecvMgr.nextProcessStep)
    {
        case DOWNLOAD_GIFT_EGG_SEND_REQUEST:
            gStringVar4[0] = 'G'; gStringVar4[1] = 'E'; gStringVar4[2] = '_'; gStringVar4[3] = '1';
            configureSendRecvMgr(NET_CONN_SCH2_REQ, (vu32 *) &gStringVar4[0], 4, NET_CONN_STATE_SEND, DOWNLOAD_GIFT_EGG_TRANSMIT_REQUEST);
            break;
        ...
        case DOWNLOAD_GIFT_EGG_RECEIVE_DATA:
            ...
            configureSendRecvMgrChunked(NET_CONN_RCHF0_REQ, (vu32 *) &gStringVar4[0], 4, NET_CONN_STATE_RECEIVE, DOWNLOAD_GIFT_EGG_FINISH, MINIMUM_CHUNK_SIZE);
            break;

        case DOWNLOAD_GIFT_EGG_FINISH:
        default:
            sSendRecvMgr.state = NET_CONN_STATE_DONE;
            gSpecialVar_0x8003 = (u16) (gStringVar4[0] | gStringVar4[1] << 8);
            gSpecialVar_0x8005 = (u16) (gStringVar4[2] | gStringVar4[3] << 8);
...
static void Task_TradeProcess(u8 taskId)
{
    switch (sSendRecvMgr.nextProcessStep)
    {
        case TRADE_CLEAR_LAST_PARTNER:
            ...
            gStringVar4[0] = 0; gStringVar4[1] = 0; gStringVar4[2] = 0; gStringVar4[3] = 0;
            configureSendRecvMgr(0x15F0, (vu32 *) &gStringVar4[0], 4, NET_CONN_STATE_SEND, TRADE_SEND_REQUEST);
            break;
        ...
        case TRADE_SEND_REQUEST:
        {
            ...
            gStringVar4[0] = 'T'; gStringVar4[1] = 'R'; gStringVar4[2] = '_';
            ...
            {
                gStringVar4[3] = '1';
                gStringVar4[4] = gSaveBlock1Ptr->easyChatProfile[2] >> 8;
                gStringVar4[5] = gSaveBlock1Ptr->easyChatProfile[2] & 0xFF;
                gStringVar4[6] = gSaveBlock1Ptr->easyChatProfile[3] >> 8;
                gStringVar4[7] = gSaveBlock1Ptr->easyChatProfile[3] & 0xFF;
                configureSendRecvMgrChunked(NET_CONN_SEND_REQ, (vu32 *) &gStringVar4[0], 8, NET_CONN_STATE_SEND, TRADE_APPEND_MON_DATA, MINIMUM_CHUNK_SIZE);
            }
            else
            {
                gStringVar4[3] = '0';
                configureSendRecvMgr(NET_CONN_SEND_REQ, (vu32 *) &gStringVar4[0], 4, NET_CONN_STATE_SEND, TRADE_APPEND_MON_DATA);
            }
            break;
        }
        ...
        case TRADE_RECEIVE_NAME_DATA:
            ...
            configureSendRecvMgrChunked(NET_CONN_RCHF0_REQ, (vu32 *) &gStringVar4[0], 16, NET_CONN_STATE_RECEIVE, TRADE_VERIFY_PARTNER_FOUND, MINIMUM_CHUNK_SIZE);
            break;
        ...
        default:
        {
            ...
            if (gSpecialVar_0x8003 != 1)
            {
                if (!(gEnemyParty[0].box.isBadEgg) && !(species > SPECIES_EGG))
                {
                    ...
                    gStringVar4[PLAYER_NAME_LENGTH + 1] = 0xFF;
...
static void Task_PostMailProcess(u8 taskId)
{
    switch (sSendRecvMgr.nextProcessStep)
    {
        case POST_MAIL_SEND_REQUEST:
        {
            ...
            u8 mailId;
            bool8 useFriendLink = TRUE;
            gStringVar4[0] = 'P'; gStringVar4[1] = 'M'; gStringVar4[2] = '_';
            ...
            {
                gStringVar4[8] = gSaveBlock1Ptr->mail[mailId].itemId >> 8;
                gStringVar4[9] = gSaveBlock1Ptr->mail[mailId].itemId & 0xFF;

                for (i = 0; i < MAIL_WORDS_COUNT; i++)
                {
                    gStringVar4[8 + 2 + (2 * i)] = gSaveBlock1Ptr->mail[mailId].words[i] >> 8;
                    gStringVar4[8 + 2 + (2 * i) + 1] = gSaveBlock1Ptr->mail[mailId].words[i] & 0xFF;
                }

                configureSendRecvMgrChunked(NET_CONN_SCH2_REQ, (vu32 *) &gStringVar4[0], 8 + 2 + (2 * 9), NET_CONN_STATE_SEND, POST_MAIL_TRANSMIT_REQUEST, MINIMUM_CHUNK_SIZE);
            }
        ...
        case POST_MAIL_RECEIVE_DATA:
            ...
            configureSendRecvMgrChunked(NET_CONN_RCHF0_REQ, (vu32 *) &gStringVar4[0], 2, NET_CONN_STATE_RECEIVE, POST_MAIL_FINISH, MINIMUM_CHUNK_SIZE);
            break;
...
static void Task_ReadMailProcess(u8 taskId)
{
    switch (sSendRecvMgr.nextProcessStep)
    {
        case READ_MAIL_CLEAR_LAST_MESSAGE:
            gStringVar4[0] = 0; gStringVar4[1] = 0; gStringVar4[2] = 0; gStringVar4[3] = 0;
            configureSendRecvMgr(0x15F0, (vu32 *) &gStringVar4[0], 4, NET_CONN_STATE_SEND, READ_MAIL_SEND_REQUEST);
            break;
        case READ_MAIL_SEND_REQUEST:
        {
            u32 i;
            bool8 useFriendLink = TRUE;
            gStringVar4[0] = 'R'; gStringVar4[1] = 'M'; gStringVar4[2] = '_';
            ...
            if (useFriendLink)
            {
                ...
                configureSendRecvMgrChunked(NET_CONN_SCH2_REQ, (vu32 *) &gStringVar4[0], 8, NET_CONN_STATE_SEND, READ_MAIL_TRANSMIT_REQUEST, MINIMUM_CHUNK_SIZE);
            }
            else
            {
                gStringVar4[3] = '0';
                configureSendRecvMgr(NET_CONN_SCH2_REQ, (vu32 *) &gStringVar4[0], 4, NET_CONN_STATE_SEND, READ_MAIL_TRANSMIT_REQUEST);
            }
            break;
        }
        ...
        case READ_MAIL_RECEIVE_DATA:
            ...
            configureSendRecvMgrChunked(NET_CONN_RCHF0_REQ, (vu32 *) &gStringVar4[0], 8 + 2 + (9 * 2), NET_CONN_STATE_RECEIVE, READ_MAIL_FINISH, MINIMUM_CHUNK_SIZE);
            break;
        ...
        default:
        {
            ...
            if (gStringVar4[0] == 0 && gStringVar4[1] == 0)
            {
                // gSpecialVar_0x8003 = 0 - Connection Error
                gSpecialVar_0x8003 = 0;
            } 
            else if (gStringVar4[0] == 0xFF && gStringVar4[1] == 0xFF)
            {
            ...
            else 
            {
                ...
                // Use gStringVar1 to store the mail data then cast it to the mail struct 
                for (i = 0; i < MAIL_WORDS_COUNT; i++)
                {
                    gStringVar1[(2 * i)] = gStringVar4[8 + 2 + (2 * i) + 1];
                    gStringVar1[(2 * i) + 1] = gStringVar4[8 + 2 + (2 * i)];

                }

                for (i = 0; i < PLAYER_NAME_LENGTH; i++)
                    gStringVar1[i + 0x12] = gStringVar4[i];

                gStringVar1[i + 0x12 + 1] = EOS;
                
                gStringVar1[0x20] = gStringVar4[8 + 1];
                gStringVar1[0x20 + 1] = gStringVar4[8];
            }
...
static void Task_EndReadMailConnection(u8 taskId) 
{
    ...
    if (gSpecialVar_0x8003 == 2)
        ReadMail((struct Mail *) &gStringVar1[0], CB2_ReturnToField, TRUE);
}